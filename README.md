...
types of groups, allow groups and deny-groups. Allow groups gives access to materials (positive Solr filter) while deny-groups
restricts access (negative Solr filter).  The more access groups a user validates will give more materials and more validated deny-groups will
also give more materials since this will remove the negative filter. The deny-groups negative filters are always applied unless
the user validates access to them and that restriction will be removed. Some materials can be locked under several different deny-groups
restrictions and all deny-groups must validate before the user can access it.
    
### Example of access with two groups
Group 1 (access group): lma_long:"radio"
    
Group 2 (deny group):  klausuleret:"ja"
...
Group 1 (access group): lma_long:"tv"
    
Group 2 (access group): lma_long:"radio"
       
Group 3 (deny group):  klausuleret:"ja"
    
Group 4 (deny group):  individuelt_forbud:"ja"
...
All the allow groups will each expand the positive filter query used for filtering Ids. All deny-group valideted will remove
the negative filter blocked by that deny-group. See 'uml/License_validation_logic.png' for a visualisation 

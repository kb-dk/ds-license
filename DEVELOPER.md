...
After a fresh checkout or after the `ds-license-openapi_v1.yaml` specification has changed, the `api` and the `model` files 
...
where "ds-license" is your artifactID from above.

The Swagger UI is available at <http://localhost:8080/ds-license/api/>, providing access to the `v1` version of the GUI. 
...
During development the configurations are located in the `conf`-folder as `conf/ds-license-behaviour.yaml`,  
`conf/ds-license-environment.yaml` and `conf/ds-license-local.yaml`. 
When the project is started using Jetty, these are the configurations that are used.

Access to the configuration is through the static class at `src/main/java/dk.kb.license/config/ServiceConfig.java`.

**Note**: The environment configuration typically contains sensitive information. Do not put it in open code
repositories. To guard against this, `conf/ds-license-environment.yaml` is added to `.gitignore`. 
...
[index.jsp](ds-license-web/src/main/webapp/index.jsp) shows, this is considered legacy technology.
...
Everything is defined centrally in the file [src/main/openapi/ds-license-openapi_v1.yaml](ds-license-web/src/main/openapi/ds-license-openapi_v1.yaml).
IntelliJ IDEA has a plugin for editing OpenAPI files that provides a semi-live preview of the generated GUI and
the online [Swagger Editor](https://editor.swagger.io/) can be used by copy-pasting the content of `ds-license-openapi_v1.yaml`.
...
A common pattern during initial definition of the `ds-license-openapi_v1.yaml` is to delay the implementation phase and recreate 
the skeleton implementation files on each build. This can be done by setting `generateOperationBody` in the `pom.xml` 
to `true`.

**Tip:** If the `ds-license-openapi_v1.yaml` is changed a lot during later development of the application, it might be better to have 
`<generateOperationBody>true</generateOperationBody>` in `pom.xml` and add the implementation code to manually created
classed (initially copied from the OpenAPI-generated skeleton impl classes). When changes to `ds-license-openapi_v1.yaml` results in
...
See how we map exceptions to responsecodes in [ServiceExceptionMapper](ds-license-web/src/main/java/dk/kb/webservice/ServiceExceptionMapper.java) 

See [ServiceException](ds-license-web/src/main/java/dk/kb/webservice/exception/ServiceException.java) and its specializations for samples.

### Mustache templates

The templates in [src/main/templates/](ds-license-web/src/main/templates/) overrides the default Swagger templates.
...
should
  ...
  * Create a new OpenAPI YAML, e.g. `ds-license-openapi_v2.yaml` (preferably by copying `ds-license-openapi_v1.yaml` and adjusting) and place
    it alongside `ds-license-openapi_v1.yaml`
    * Edit the old `ds-license-openapi_v1.yaml` and add a note that a new version is available
  ...
  * Add the path to the new OpenAPI YAML to `urls` in `webapp/api/index.html`. The first entry in the array is the
    default when visiting the main API-page at <http://localhost:8080/ds-license/api/>  
...
```
[Unreleased](https://github.com/kb-dk/ds-license-template/compare/v1.0.0...HEAD)
[1.1.0](https://github.com/kb-dk/ds-license-template/compare/v1.0.0...v1.1.0)
[1.0.0](https://github.com/kb-dk/ds-license-template/releases/tag/v1.0.0)
```

For BitBucket (KB's internal git), the syntax is
```
[Unreleased](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/ds-license-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.1.0&sourceBranch=refs%2Fheads%2Fmaster)
[1.1.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/ds-license-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.0.0&sourceBranch=refs%2Ftags%2Fv1.1.0)
[1.0.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/ds-license-template/commits?until=refs%2Ftags%2Fv1.0.0)
```
Note that both the `ARK`-part and the repo-id `ds-license-template` is project-specific.


## Release procedure

1. Review that the `version` in `pom.xml` is fitting. `ds-license` uses
...
which boils down to
   ...
   * Run `git push`   
   ...
   `mvn release:rollback ; git tag -d ds-license-1.4.2 ; git push --delete origin ds-license-1.4.2`

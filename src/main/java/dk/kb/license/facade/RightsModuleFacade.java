...
public class RightsModuleFacade {
     ...
     * @throws Exception
     */
    public static void deleteRestrictedId(String id, String idType, PlatformEnumDto platform, String user, boolean touchDsStorageRecord) throws Exception {
        BaseModuleStorage.performStorageAction("delete restricted ID",RightsModuleStorage.class, storage -> {
            ((RightsModuleStorage) storage).deleteRestrictedId(id, idType, platform.getValue());
            if (touchDsStorageRecord) {
                touchRelatedStorageRecords(id, idType);
            }

            ChangeDifferenceText change = RightsChangelogGenerator.deleteRestrictedIdChanges(id,idType,platform.getValue());
    ...
    public static void deleteRestrictedIds(List<RestrictedIdInputDto> restrictedIds, String user, boolean touchDsStorageRecord) {
        BaseModuleStorage.performStorageAction("delete restricted ID",RightsModuleStorage.class, storage -> {
            for(RestrictedIdInputDto id : restrictedIds) {
                ...
                ChangeDifferenceText change = RightsChangelogGenerator.deleteRestrictedIdChanges(id.getIdValue(),id.getIdType(),id.getPlatform().getValue());

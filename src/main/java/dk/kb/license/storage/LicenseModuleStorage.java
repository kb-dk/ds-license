 ...
 *  PRESENTATIONTYPES: configured presentationtypes.
 *  GROUPTYPES: configured groups
 *  ATTRIBUTETYPES: configured attributes
 * 
...
public class LicenseModuleStorage  implements AutoCloseable{
    ...
    private final static String selectDomLicensePresentationTypesQuery = " SELECT * FROM " + DOMLICENSEPRESENTATIONTYPES_TABLE;
    ...
    private final static String selectAttributeGroupsForLicenseQuery = " SELECT * FROM " + ATTRIBUTEGROUP_TABLE + " WHERE " + LICENSEID_COLUMN + "= ? ORDER BY NUMBER";

    private final static String persistAttributeForAttributeGroupQuery = "INSERT INTO " + ATTRIBUTE_TABLE + " (" + ID_COLUMN + "," + NAME_COLUMN + "," + ATTRIBUTEGROUPID_COLUMN + ") VALUES (?,?,?)"; // #|?|=3

    private final static String selectAttributesForAttributeGroupQuery = " SELECT * FROM " + ATTRIBUTE_TABLE + " WHERE " + ATTRIBUTEGROUPID_COLUMN + " = ?";

    private final static String persistValueForAttributeQuery = "INSERT INTO " + VALUE_TABLE + " (" + ID_COLUMN + "," + VALUE_COLUMN + "," + ATTRIBUTEID_COLUMN + ") VALUES (?,?,?)"; // #|?|=3

    private final static String selectValuesForAttributeQuery = " SELECT * FROM " + VALUE_TABLE + " WHERE " + ATTRIBUTEID_COLUMN + " = ?";

    private final static String selectDomLicenseGroupTypesQuery = " SELECT * FROM " + DOMLICENSEGROUPTYPES_TABLE + " ORDER BY " + KEY_COLUMN;

    private final static String persistDomLicenseGroupTypeQuery = "INSERT INTO " + DOMLICENSEGROUPTYPES_TABLE + " (" + ID_COLUMN + "," + KEY_COLUMN + "," + VALUE_DK_COLUMN + " ," + VALUE_EN_COLUMN
            + " ," + DESCRIPTION_DK_COLUMN + " ," + DESCRIPTION_EN_COLUMN + " ," + QUERY_COLUMN + " ," + MUSTGROUP_COLUMN + ") VALUES (?,?,?,?,?,?,?,?)"; // #|?|=8

    private final static String updateDomLicenseGroupTypeQuery = "UPDATE " + DOMLICENSEGROUPTYPES_TABLE + " SET " + VALUE_DK_COLUMN + " = ? , " + VALUE_EN_COLUMN + " = ? ," + DESCRIPTION_DK_COLUMN
            + " = ? ," + DESCRIPTION_EN_COLUMN + " = ? ," + QUERY_COLUMN + " = ? ," + MUSTGROUP_COLUMN + " = ? " + "WHERE " + ID_COLUMN + " = ? ";

    private final static String updateDomsLicensePresentationTypeQuery = "UPDATE " + DOMLICENSEPRESENTATIONTYPES_TABLE + " SET " + VALUE_DK_COLUMN + " = ? , " + VALUE_EN_COLUMN + " = ? " + "WHERE "
            + ID_COLUMN + " = ? ";

    private final static String persistLicenseQuery = "INSERT INTO " + LICENSE_TABLE + " (" + ID_COLUMN + "," + NAME_COLUMN + "," + NAME_EN_COLUMN + "," + DESCRIPTION_DK_COLUMN + ","
            + DESCRIPTION_EN_COLUMN + "," + VALIDFROM_COLUMN + "," + VALIDTO_COLUMN + ") VALUES (?,?,?,?,?,?,?)"; // #|?|=7

    private final static String selectDomAttributeTypesQuery = " SELECT * FROM " + DOMATTRIBUTETYPES_TABLE + " ORDER BY " + VALUE_COLUMN;;

    private final static String deleteDomAttributeTypeByNameQuery = " DELETE FROM " + DOMATTRIBUTETYPES_TABLE + " WHERE " + VALUE_COLUMN + " = ?";

    private final static String deleteDomGroupTypeByKeyQuery = " DELETE FROM " + DOMLICENSEGROUPTYPES_TABLE + " WHERE " + KEY_COLUMN + " = ?";

    private final static String deleteDomPresentationTypeByKeyQuery = " DELETE FROM " + DOMLICENSEPRESENTATIONTYPES_TABLE + " WHERE " + KEY_COLUMN + " = ?";

    private final static String persistDomAttributeTypeQuery = "INSERT INTO " + DOMATTRIBUTETYPES_TABLE + " (" + ID_COLUMN + "," + VALUE_COLUMN + ") VALUES (?,?)"; // #|?|=2

    private final static String selectLicenseContentForLicenseQuery = " SELECT * FROM " + LICENSECONTENT_TABLE + " WHERE " + LICENSEID_COLUMN + " = ? ";

    private final static String persistLicenseContentForLicenseQuery = "INSERT INTO " + LICENSECONTENT_TABLE + " (" + ID_COLUMN + "," + NAME_COLUMN + "," + LICENSEID_COLUMN + ") VALUES (?,?,?)"; // #|?|=3

    private final static String selectPresentationTypesForLicenseContentQuery = " SELECT * FROM " + PRESENTATION_TABLE + " WHERE " + LICENSECONTENTID_COLUMN + " = ? ";

    private final static String persistPresentationTypesForLicenseContentQuery = "INSERT INTO " + PRESENTATION_TABLE + " (" + ID_COLUMN + "," + NAME_COLUMN + "," + LICENSECONTENTID_COLUMN
            + ") VALUES (?,?,?)"; // #|?|=3

    // Deletes
    private final static String deletePresentationsByLicenseContentIdQuery = " DELETE FROM " + PRESENTATION_TABLE + " WHERE " + LICENSECONTENTID_COLUMN + " = ?";

    private final static String deleteLicenseContentsByLicenseIdQuery = " DELETE FROM " + LICENSECONTENT_TABLE + " WHERE " + LICENSEID_COLUMN + " = ?";

    private final static String deleteAttributesByAttributeGroupIdQuery = " DELETE FROM " + ATTRIBUTE_TABLE + " WHERE " + ATTRIBUTEGROUPID_COLUMN + " = ?";

    private final static String countAttributesByAttributeNameQuery = " SELECT COUNT(*) FROM " + ATTRIBUTE_TABLE + " WHERE " + NAME_COLUMN + " = ?";

    private final static String countGroupTypeByGroupNameQuery = " SELECT COUNT(*) FROM " + LICENSECONTENT_TABLE + " WHERE " + NAME_COLUMN + " = ?";

    private final static String countPresentationTypeByPresentationNameQuery = " SELECT COUNT(*) FROM " + PRESENTATION_TABLE + " WHERE " + NAME_COLUMN + " = ?";

    private final static String deleteValuesByAttributeIdQuery = " DELETE FROM " + VALUE_TABLE + " WHERE " + ATTRIBUTEID_COLUMN + " = ?";
    ...
    public void persistDomLicensePresentationType(String key, String value_dk, String value_en) throws Exception {
        ...
        try {
            stmt = connection.prepareStatement(persistDomLicensePresentationTypeQuery);
    ...
    public ArrayList<ConfiguredDomLicensePresentationType> getDomLicensePresentationTypes() throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectDomLicensePresentationTypesQuery);
    ...
    public void deleteLicense(long licenseId, boolean commit) throws Exception {
    ...
    public void updateDomPresentationType(long id, String value_dk, String value_en) throws Exception {
        ...
        try {
            ...
            // if it exists already, we do not add it.
            stmt = connection.prepareStatement(updateDomsLicensePresentationTypeQuery);
            stmt.setString(1, value_dk);
            stmt.setString(2, value_en);
            stmt.setLong(3, id);
    ...
    public void deleteDomLicenseGroupType(String groupName) throws Exception {
        ...
        PreparedStatement stmt = null;
        try {
            stmt = connection.prepareStatement(countGroupTypeByGroupNameQuery);
            ...
            stmt = connection.prepareStatement(deleteDomGroupTypeByKeyQuery);
            stmt.setString(1, groupName);
            int updated = stmt.executeUpdate();
    ...
    public void deleteDomPresentationType(String presentationName) throws Exception {
        ...
        PreparedStatement stmt = null;
        try {
            stmt = connection.prepareStatement(countPresentationTypeByPresentationNameQuery);
            ...
            stmt = connection.prepareStatement(deleteDomPresentationTypeByKeyQuery);
            stmt.setString(1, presentationName);
            int updated = stmt.executeUpdate();
    ...
    public void persistLicense(License license) throws Exception {
        ...
        try {
            ...
            stmt = connection.prepareStatement(persistLicenseQuery);
            ...
            connection.commit(); // This commit will also commit all the nested statements
        } catch (Exception e) {            
            log.error("SQL Exception in persistLicense:" + e.getMessage());
            log.error("Beginning rollback of transaction");
            connection.rollback(); // This is the only situation we have to do rollback due to multiple statements.
    ...
    public ArrayList<ConfiguredDomLicenseGroupType> getDomLicenseGroupTypes() throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectDomLicenseGroupTypesQuery);
            ResultSet rs = stmt.executeQuery();
    ...
    public void persistDomAttributeType(String value) throws Exception {
        ...
        PreparedStatement stmt = null;
        try {
            stmt = connection.prepareStatement(persistDomAttributeTypeQuery);
            stmt.setLong(1, generateUniqueID());
    ...
    public  void deleteDomAttributeType(String attributeTypeName) throws Exception {
        ...
        PreparedStatement stmt = null;
        try {
            stmt = connection.prepareStatement(countAttributesByAttributeNameQuery);
            ...
            stmt = connection.prepareStatement(deleteDomAttributeTypeByNameQuery);
            stmt.setString(1, attributeTypeName);
            int updated = stmt.executeUpdate();
    ...
    public ArrayList<ConfiguredDomAttributeType> getDomAttributeTypes() throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectDomAttributeTypesQuery);
            ResultSet rs = stmt.executeQuery();
    ...
    public ArrayList<License> getAllLicenseNames() throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectAllLicensesQuery);
            ResultSet rs = stmt.executeQuery();
    ...
    public License getLicense(long licenseId) throws Exception {
        ...
        License license = new License();
        try {
            stmt = connection.prepareStatement(selectLicenseQuery);
            stmt.setLong(1, licenseId);
            ResultSet rs = stmt.executeQuery();
    ...
    protected void persistAttributeGroupsForLicense(Long licenseId, ArrayList<AttributeGroup> attributegroups, boolean commit) throws SQLException {
        ...
        try {
            ...
            for (AttributeGroup current : attributegroups) {
                ...
                stmt = connection.prepareStatement(persistAttributeGroupForLicenseQuery);
    ...
    protected ArrayList<AttributeGroup> getAttributeGroupsForLicense(Long licenseId) throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectAttributeGroupsForLicenseQuery);
            stmt.setLong(1, licenseId);
            ResultSet rs = stmt.executeQuery();
    ...
    protected void persistAttributesForAttributeGroup(Long attributeGroupId, ArrayList<Attribute> attributes, boolean commit) throws SQLException {
        ...
        try {
            ...
            for (Attribute current : attributes) {
                ...
                stmt = connection.prepareStatement(persistAttributeForAttributeGroupQuery);
    ...
    protected ArrayList<LicenseContent> getLicenseContentsForLicense(Long licenseId) throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectLicenseContentForLicenseQuery);
            stmt.setLong(1, licenseId);
            ResultSet rs = stmt.executeQuery();
    ...
    protected void persistLicenseContentsForLicense(Long licenseId, ArrayList<LicenseContent> licenseContents, boolean commit) throws SQLException {
        ...
        try {
            ...
            for (LicenseContent current : licenseContents) {
                ...
                stmt = connection.prepareStatement(persistLicenseContentForLicenseQuery);
    ...
    protected void persistPresentationsForLicenseContent(Long licenseContentId, ArrayList<Presentation> presentations, boolean commit) throws SQLException {
        ...
        try {
            ...
            for (Presentation current : presentations) {
                stmt = connection.prepareStatement(persistPresentationTypesForLicenseContentQuery);
                stmt.setLong(1, generateUniqueID());
    ...
    protected ArrayList<Presentation> getPresentationsForLicenseContent(long licenseContentId) throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectPresentationTypesForLicenseContentQuery);
            stmt.setLong(1, licenseContentId);
            ResultSet rs = stmt.executeQuery();
    ...
    protected ArrayList<Attribute> getAttributesForAttributeGroup(long attributeGroupId) throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectAttributesForAttributeGroupQuery);
            stmt.setLong(1, attributeGroupId);
            ResultSet rs = stmt.executeQuery();
    ...
    protected void persistValuesForAttribute(Long attributeId, ArrayList<AttributeValue> values, boolean commit) throws SQLException {
        ...
        try {
            ...
            for (AttributeValue current : values) {
                stmt = connection.prepareStatement(persistValueForAttributeQuery);
                stmt.setLong(1, generateUniqueID());
    ...
    protected ArrayList<AttributeValue> getValuesForAttribute(long attributeId) throws SQLException {
        ...
        try {
            stmt = connection.prepareStatement(selectValuesForAttributeQuery);
            stmt.setLong(1, attributeId);
            ResultSet rs = stmt.executeQuery();

...
public class RightsCalculation {
     ...
     * @param drOutput                   the output data transfer object where the DR holdback name and expiration date
    ...
    private static void setHoldbackForRadioRecordDrArchive(RightsCalculationInputDto rightsCalculationInputDto, RightsCalculationOutputDrDto drOutput) {
        String holdbackExpiredDate = calculateDrHoldbackDateByYears(rightsCalculationInputDto.getStartTime(), ServiceConfig.getDrHoldbackYearsForRadio());
    ...
    private static String getDrHoldbackExpiredDate(DrHoldbackRuleOutputDto holdbackRule, String recordId, String startDate) {
        ...
        if (holdbackDays < ServiceConfig.getDrHoldbackLogicChangeDays()) {
            ...
            return calculateDrHoldbackDateByDays(startDate, holdbackDays);
        } else {
            ...
            return calculateDrHoldbackDateByYears(startDate, holdbackYears);
        }
    }
     ...
     */
    private static String calculateDrHoldbackDateByDays(String startDate, int holdbackDays) {
     ...
     * @return the date, when the holdback period has expired as a string in the format: yyyy-MM-dd'T'HH:mm:ssZ.
     */
    private static String calculateDrHoldbackDateByYears(String startDate, int holdbackYears) {
        ZonedDateTime cleanedStartDate = getCleanZonedDateTimeFromString(startDate);

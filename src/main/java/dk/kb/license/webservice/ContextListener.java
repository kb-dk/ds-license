...
public class ContextListener implements ServletContextListener {
    ...
    public static final String LOGBACK_ENV = "java:/comp/env/ds-license-logback-config";
    ...
    public void contextInitialized(ServletContextEvent sce) {
    	...
    	try {
            ...
            if (mxBean.getInputArguments().stream().noneMatch(arg -> arg.startsWith("-Xmx"))) {
                log.warn("Java heap size (-Xmx option) is not specified. In stage or production this is almost always an error");
            }
            ...
            initialiseStorage();  
            log.info("ServiceConfig initialized config glob '{}'", configFile);
        }

...
public class DsRightsApiServiceImpl extends ImplBase implements DsRightsApi {
    private static final Logger log = LoggerFactory.getLogger(DsRightsApiServiceImpl.class);
    ...
    public void createRestrictedId(RestrictedIdInputDto restrictedIdInputDto) {
        ...
        try {
            BaseModuleStorage.performStorageAction("Persist restricted ID (klausulering)", new RightsModuleStorage(), storage -> {
                ((RightsModuleStorage) storage).persistRestrictedId(
                        restrictedIdInputDto.getId(),
                        restrictedIdInputDto.getIdType(),
                        restrictedIdInputDto.getSystem(),
    ...
    @Override
    public void deleteRestrictedId(String id, String idType, String system) {
        try {
            BaseModuleStorage.performStorageAction("delete restricted ID",new RightsModuleStorage(), storage -> {
                ((RightsModuleStorage) storage).deleteRestrictedId(id, idType, system);
                return null;
            });
        } catch (Exception e) {
            handleException(e);
    ...
    @Override
    public RestrictedIdOutputDto getRestrictedId(String id, String idType, String system) {
        try {
            ...
            return BaseModuleStorage.performStorageAction("Get restricted ID", new RightsModuleStorage(), storage -> {
                RestrictedIdOutputDto output = ((RightsModuleStorage)storage).getRestrictedId(id, idType, system);
                if (output != null) {
                    return output;
                }
